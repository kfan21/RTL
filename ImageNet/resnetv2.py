...
class ResNetV2(nn.Module):
    ...
    def forward(self, x, layer_index=None, return_feature = False):
        ...
        feature = self.before_head(self.body(self.root(x)))
        x = self.head(feature)
        assert x.shape[-2:] == (1, 1)  # We should have no spatial shape left.
        if return_feature:
            return feature.reshape(feature.shape[0], -1), x[..., 0, 0]
        else:
            return x[..., 0, 0]
        
        

    def forward_with_feature(self, x, layer_index=None):
        if layer_index is not None:
            return self.intermediate_forward(x, layer_index)
    ...
    def feature(self, x):
        return self.before_head(self.body(self.root(x)))

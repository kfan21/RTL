...
#kefan
def get_ood_scores_odin_rtl(loader, net, bs, ood_num_examples, T, noise, in_dist=False):
    _score = []
    _feature = []
    _right_score = []
    _wrong_score = []
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        odin_feature, odin_score = ODIN_RTL(data, output,net, T, noise)
        _score.append(-np.max(odin_score, 1))
        _feature.append(to_np(odin_feature))
...
def ODIN_RTL(inputs, outputs, model, temper, noiseMagnitude1):
    ...
    features, outputs = model.forward_with_feature(Variable(tempInputs))
    outputs = outputs / temper
